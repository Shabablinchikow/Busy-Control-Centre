...
final class StatusApp: MiniApp {
    ...
    /// Redrawn on this cadence so the banner comes back on its own after
    /// something else has had the screen.
    static let refresh = 5.0
    ...
    func run(client: BarClient, status: @escaping @Sendable (String) -> Void) async {
        ...
        while !Task.isCancelled {
            let code = (try? await client.draw(app: app,
                                               elements: [animationEl("banner", stock: stock)])) ?? 0
            switch code {
            case 200: status("showing \(title)")
            case 409: status("paused — bar in use")
            default: status("the bar refused the banner (HTTP \(code))")
            }
            if !(await barSleep(Self.refresh)) { break }
        }
